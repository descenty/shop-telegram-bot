...
class User:
    ...
    async def _query(self, field: str) -> Any:
        return (
            await database.fetch(
                f"SELECT {field} FROM users WHERE id = ?", self.id
            )
        )[0][0]

    async def _update(self, field: str, value: Any) -> None:
        await database.fetch(
            f"UPDATE users SET {field} = ? WHERE id = ?", value, self.id
        )
    ...
    async def date_created(self) -> datetime.datetime:
        return datetime.datetime.strptime(
            await self._query("date_created"), constants.TIME_FORMAT
        )
    ...
    async def orders(self) -> list[orders.Order]:
        return [
            ...
            for order_id in await database.fetch(
                f"SELECT id FROM orders WHERE user_id = ?", self.id
            )
        ]
...
async def get_users() -> list[User]:
    return [
        User(*user_id)
        for user_id in await database.fetch("SELECT id FROM users")
    ]
...
async def create(user_id: int, username: str) -> None:
    await database.fetch(
        ...
        json.dumps({"items": {}, "delivery": 0, "payment": 0}),
    )
