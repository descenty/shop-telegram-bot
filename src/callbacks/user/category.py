...
async def execute(
    ...
    name, children, items, parent_id = await asyncio.gather(
        category.name, category.children, category.items, category.parent_id
    )
    ...
    for item in items:
        ...
        markup.append(
            (f"{price} ₽ {name}", f'{{"r":"user","iid":{item.id}}}item')
        )
    if parent_id:
        markup.append(
            (
        ...
        markup.append(
            (constants.language.back, f"{constants.JSON_USER}catalogue")
        )

    text = await category.name
