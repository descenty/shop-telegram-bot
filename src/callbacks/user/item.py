...
async def execute(
    ...
    def cart_callback(state: int):
        return (
            f'{{"r":"user","iid":{item.id},"s":{state},"d":"item"}}changeCart'
        )
    ...
    back_button = (
        (
            ...
            f'{{"r":"user","cid":"{category.id}"{del_data if image_id else ""}}}category',
        ),
    )
    ...
    if "rd" in data:
        back_button = (
            constants.language.back,
            f"{constants.JSON_USER}{data['rd']}",
        )
    markup = markups.create([cart_buttons, back_button])
    ...
    if image_id:
        ...
        return await callback_query.message.answer_photo(
            photo=image_id, caption=item_text, reply_markup=markup
        )
    return await callback_query.message.edit_text(
        text=item_text, reply_markup=markup
    )
